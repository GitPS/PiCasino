        ...
        switch( gameState.getPhase() ){
            ...
            case BETTING:
                ...
                if( isValid( gameState.getPhase(), input ) ){
                    BJGameEvent event = new BJGameEvent();
                    event.setName(input);
                    if( input.equals("AdvancePhase") || input.equals("Pass") ){
                        gameState.invoke( event );
                    } else {
            ...
            case PLAYING:
                ...
                if( isValid( gameState.getPhase(), input ) ){
                    BJGameEvent event = new BJGameEvent();
                    event.setName(input);
                    if( input.equals("AdvancePhase") || input.equals("Stay") || input.equals("DoubleDown") || input.equals("Split") ){
                        gameState.invoke( event );
    ...
    private void printStatus(){
        ...
        for(BJClientGameState.Hand h: gameState.getHands()){
            ...
            for( Integer i : h.getCards() )
                toPrint += "[ " + i % 13 + " of "+evaluateCardSuit(i)+"s ]";
